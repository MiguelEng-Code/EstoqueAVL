...
•	Principais Funcionalidades:
  o	Interface Gráfica Moderna: Telas desenhadas com Scene Builder, utilizando design responsivo e ícones.
  o	CRUD de Produtos: Cadastro de itens com validação de dados (ID numérico, preço, etc.).
  o	Busca Otimizada: Localização instantânea de produtos pelo ID ou filtragem por nome.
  o	Persistência de Dados: O sistema utiliza a biblioteca GSON para salvar os dados automaticamente em um arquivo JSON (estoque_dados.json) ao fechar a aplicação, e recarregá-los ao iniciar, garantindo que nenhum registro seja perdido.
